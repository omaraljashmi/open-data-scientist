...
- [x] Pass the deterministic 100,000-row benchmark in 1.318 seconds in the validation environment.

## CI and deployment gate

- [ ] Python 3.11 test job passes on GitHub Actions.
- [ ] Python 3.12 test job passes on GitHub Actions.
- [ ] Docker image builds on GitHub Actions.
- [ ] Docker container reaches the Streamlit health endpoint.
- [ ] Hosted Streamlit redeploys from the approved commit.
- [ ] Hosted sample walkthrough completes without an exception.

Release-candidate run `29387684032` built the Docker image successfully, but the first health probe reached the starting container after roughly 0.7 seconds and exited with curl error 56 (`Connection reset by peer`). The focused CI fix retries all temporary startup errors, prints container logs on failure, and always removes the test container. The gate remains open until GitHub Actions verifies that fix.

## Stable promotion

- [ ] Resolve every release-blocking issue and record the verification evidence.
- [ ] Change version constants from `1.0.0rc1` / `v1.0.0-rc.1` to `1.0.0` / `v1.0.0`.
- [ ] Move the validated changes from **Unreleased** into the dated `1.0.0` changelog section.
- [ ] Finalize `docs/releases/v1.0.0.md` from the verified results.
