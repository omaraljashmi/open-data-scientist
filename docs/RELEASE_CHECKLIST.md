...
- [ ] Docker image builds and its `/_stcore/health` endpoint responds.
- [x] Privacy, security, input limits, install paths, and known boundaries are documented.

The local verification environment does not provide a container engine. The Docker build and health check remain an explicit GitHub Actions gate after publication.

## Publication gate

- [ ] Review the exact GitHub diff; exclude unrelated local worktree state.
- [ ] Publish the release-candidate files to `main` only after explicit approval.
- [ ] Confirm both GitHub Actions jobs pass on `main`.
- [ ] Confirm the hosted Streamlit app redeploys and the sample walkthrough works.
- [ ] Create annotated tag `v1.0.0-rc.1` only after separate approval.
- [ ] Create the GitHub prerelease from `docs/releases/v1.0.0-rc.1.md` only after separate approval.
- [ ] Record any release-blocking issue before promoting to `v1.0.0`.
